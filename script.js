document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderCalendar = (weekStartDate) => {
        ...
        const weekData = allScheduleData.filter(item => {
            ...
            // Check status for recurring logic
            const status = (item.Status || '').toLowerCase().trim();

            if (status === 'ulang') {
                // For "ulang" status, show in the same day of week and session every week
                const itemDayOfWeek = itemDate.getDay(); // 0 = Sunday, 1 = Monday, etc.
                const currentWeekDays = [];

                // Generate all days in current week
                for (let i = 0; i < 7; i++) {
                    const dayInWeek = new Date(weekStartDate);
                    dayInWeek.setDate(weekStartDate.getDate() + i);
                    currentWeekDays.push(dayInWeek);
                }

                // Check if any day in current week matches the original day of week
                const matchingDay = currentWeekDays.find(day => day.getDay() === itemDayOfWeek);
                if (matchingDay && matchingDay >= weekStartDate && matchingDay <= weekEndDate) {
                    console.log('🔄 Recurring item matched for current week:', {
                        pengajar: item.Pengajar,
                        ruang: item.Ruang,
                        originalDate: item.Tanggal,
                        matchingDate: matchingDay.toDateString(),
                ...
                // For "sekali" or empty status, use exact date matching
        ...
        Object.keys(SESSIONS).forEach(sessionKey => {
            ...
            DAYS.forEach((_, dayIndex) => {
                ...
                const schedules = weekData.filter(item => {
                    ...
                    if (status === 'ulang') {
                        // For recurring items, match by day of week instead of exact date
                        const itemDayOfWeek = itemDate.getDay();
                        const currentDayOfWeek = currentDay.getDay();
                        dateMatch = itemDayOfWeek === currentDayOfWeek;

                        if (dateMatch && sesiMatch) {
                            console.log('🔄 Recurring schedule item matched:', {
                                pengajar: item.Pengajar,
                                ruang: item.Ruang,
                                originalDate: item.Tanggal,
                                currentDate: currentDay.toDateString(),
                                dayOfWeek: itemDayOfWeek,
                                sesi: item.Sesi,
                                status: item.Status
                            });
                        ...
                        // For one-time items, use exact date matching
                ...
                if (schedules.length > 0) {
                    schedules.forEach(schedule => {
                        ...
                        // Add tambah class for styling
                        if (isTambah) {
                            itemDiv.classList.add('schedule-tambah');
                            console.log('🎨 Added schedule-tambah class to:', schedule.Pengajar);
                        }
                        ...
                        // Add TAMBAH+ stamp if status is tambah
                        if (isTambah) {
                            htmlContent += `<div class="tambah-stamp">TAMBAH+</div>`;
                            console.log('✅ Added TAMBAH+ stamp for:', {
                                pengajar: formattedPengajar,
                                ruang: ruang,
                                keterangan: schedule.Keterangan
                            });
    ...
    const fetchDataAndRender = async (preserveCurrentWeek = false) => {
        ...
        try {
            ...
            // Debug: Log all status values before filtering (including empty ones)
            const allStatus = allScheduleData.map(item => item.Status || '[EMPTY]');
            console.log('📋 All Status values found (including empty):', [...new Set(allStatus)]);
            ...
            // Debug: Check for tambah records
            const tambahRecords = allScheduleData.filter(item => {
                const keterangan = (item.Keterangan || '').toLowerCase().trim();
                return keterangan === 'tambah';
            });
            console.log('➕ Found tambah records:', tambahRecords.length, tambahRecords);

            // Debug: Check for ulang status records
            const ulangRecords = allScheduleData.filter(item => {
                const status = (item.Status || '').toLowerCase().trim();
                return status === 'ulang';
            });
            console.log('🔄 Found ulang status records:', ulangRecords.length, ulangRecords);

            // Debug: Check for sekali status records
            const sekaliRecords = allScheduleData.filter(item => {
                const status = (item.Status || '').toLowerCase().trim();
                return status === 'sekali';
            });
            console.log('1️⃣ Found sekali status records:', sekaliRecords.length, sekaliRecords);
